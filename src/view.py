...
class BasicTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log = logger.logger("viewtest")
    ...
    def _query(self, db, view_name, map, reduce=None, **params):
        ...
        return db.view('test/' + view_name, **params)

    def _test_collation(self):
        db_name = "test_collation"
        try:
        ...
        values.append({"b":1})
        values.append({"b":2})
        values.append({"b":2, "a":1})
        
        for i in range(len(values)):
            db.save_doc({"_id": str(i), "value": values[i]})

        query = """function(doc) {
            emit(doc.value, doc.id);
        }"""

        rows = self._query(db, "collation", query, endkey="b", descending=True, inclusive_end=True)
        rows = self._query(db, "collation", query, endkey="b", descending=True, inclusive_end=False)
        rows = self._query(db, "collation", query, endkey="b", endkey_docid="10", descending=True, inclusive_end=False)
    ...
    def test_multi_key(self):
        ...
        try:
            ...
            pass
        db = self.server.get_or_create_db(db_name)
        docs = self._random_doc(100)
        db.bulk_save(docs)
        keys = ["10", "15", "30", "37", "50"]
        rows = db.all_docs(keys=keys)
        tools.eq_(len(keys), len(rows))
        rows = db.all_docs(limit=1, keys=keys)
        tools.eq_(rows.total_rows, 1)
